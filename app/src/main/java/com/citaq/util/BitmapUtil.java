...
public class BitmapUtil {
	private final static int WIDTH = 552; //384;
	...
	public static Bitmap StringListtoBitmap(Context context, ArrayList<StringBitmapParameter> AllString) {
	      ...
	      //  canvas.save(Canvas.ALL_SAVE_FLAG);  //在androidP上会报错，修改方法：
	        canvas.save();
	...
	/***
	 * 使用两个方法的原因是： logo标志需要居中显示，如果直接使用同一个方法是可以显示的，但是不会居中
	 */
	...
	public static Bitmap adjustSize(Bitmap mBitmap) {
		int width = Math.min(mBitmap.getWidth(), WIDTH);
		int startWidth = (width - mBitmap.getWidth()) / 2;
		int height = mBitmap.getHeight() ;
		Bitmap result = Bitmap.createBitmap(width, height,
				Bitmap.Config.RGB_565);
//		Bitmap.createScaledBitmap(mBitmap, 800, height, true);
		...
		Canvas canvas = new Canvas(result);
		canvas.drawBitmap(mBitmap, startWidth, 0, null);
		
//		canvas.drawBitmap(Bitmap.createScaledBitmap(mBitmap, width, height, true), startWidth, 0, null);
		;
