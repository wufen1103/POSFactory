...
public class Usb {
    ...
    public final static int USB_PRINTER_VENDOR_ID = 8401;   // VID while in Printer mode
    public final static int USB_PRINTER_PRODUCT_ID = 28680; // PID while in Printer mode
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_USB_PERMISSION.equals(action)) {
                ...
                synchronized (this) {
                    ...
                    requestPermission(mContext, USB_PRINTER_VENDOR_ID, USB_PRINTER_PRODUCT_ID);
                }
    ...
    public boolean sentData(byte[] data) {
        ...
        for (int i = 0; i < mDevice.getInterfaceCount(); i++) {
            ...
            for (int j = 0; j < usbInterface.getEndpointCount(); j++) {
                ...
                if (ep.getDirection() == UsbConstants.USB_DIR_OUT) {//OUT endpoint: from Host to USB device
                    if (mConnection.bulkTransfer(ep, data, data.length, 0) >= 0) {
                        //0 或者正数表示成功
                        return true;
                    } else {
                    }
